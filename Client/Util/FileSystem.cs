...
namespace dCForm.Client.Util
{
    public static class FileSystem
    {
        ...
        /// 
        /// </summary>
        /// <param name="srcPath">single file path or directory to aggregate multiple file paths</param>
        /// <returns></returns>
        public static string calcMd5(string srcPath)
        {
            string[] filePaths = File.Exists(srcPath)
                                     ? new[] { srcPath }
                                     : Directory.GetFiles(srcPath, "*", SearchOption.AllDirectories).OrderBy(p => p).ToArray();
