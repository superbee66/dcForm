...
namespace dCForm.Util.Xsd
{
    public class Xsd
    {
        ...
        private static void GenerateVersionComment(CodeNamespace codeNamespace)
        {
            ...
            codeNamespace.Comments.Add(new CodeCommentStatement(Res.GetString("InfoVersionComment", name.Name, "4.0.30319.1")));
            codeNamespace.Comments.Add(new CodeCommentStatement(""));
        }
        ...
        private static string GetPathFromUri(Uri uri)
        {
            if (uri != null)
                ...
                {
                    if (ex is ThreadAbortException || ex is StackOverflowException || ex is OutOfMemoryException || ex is ConfigurationException) throw;
                }
            return null;
        }
        ...
        public string ImportSchemasAsClasses(string[] DocXsds, string ns, CodeGenerationOptions options, StringCollection schemaImporterExtensions)
        {
            ...
            for (int i = 0; i < DocXsds.Length; i++)
                pseudoFileStreams[string.Format("http://schemas.progablab.com/2014/12/Serialization/pseudoFile_{0}.xsd", i).ToLower()] = DocXsds[i];
        ...
        {
            Console.WriteLine(Res.GetString("Warning", message));
        }

        public static void XsdParametersValidationHandler(object sender, ValidationEventArgs args)
        {
            string text = Res.GetString("XsdParametersValidationError", "http://microsoft.com/dotnet/tools/xsd/", args.Message);
            if (args.Exception.LineNumber != 0 || args.Exception.LinePosition != 0)
                text = text + " " + Res.GetString("ErrorPosition", args.Exception.LineNumber.ToString(CultureInfo.InvariantCulture), args.Exception.LinePosition.ToString(CultureInfo.InvariantCulture));
